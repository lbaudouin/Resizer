...
public:
    ...
    void notifyPrimary()
    {
        ...
        QStringList args = q_ptr->arguments();
        args.removeFirst();

        for(int i=0;i<args.size();i++){
            QFileInfo info(args.at(i));
            if(info.exists()){
                args[i] = info.absoluteFilePath();
            }
        }
 ...
 * @param argv
 */
SingleApplication::SingleApplication(int &argc, char *argv[], quint8 secondaryInstances )
