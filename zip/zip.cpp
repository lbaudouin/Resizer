...
{
    d->addEntry(ZipWriterPrivate::File, fileName, data);
}
...
void ZipWriter::addFile(const QString &fileName, QIODevice *device)
{
    ...
    d->addEntry(ZipWriterPrivate::File, fileName, device->readAll());
    if (opened)
        device->close();
}
...
{
    d->addEntry(ZipWriterPrivate::Symlink, fileName, QFile::encodeName(destination));
}
